...
export const updateDiscussion = ({ data, discussionId }: UpdateDiscussionDTO): Promise<Discussion> => {
...
export const useUpdateDiscussion = ({ config }: UseUpdateDiscussionOptions = {}) => {
  ...
  return useMutation({
    onMutate: async (updatingDiscussion: any) => {
      ...
      const previousDiscussion = queryClient.getQueryData<Discussion>(["discussion", updatingDiscussion?.discussionId]);
    ...
    onError: (_, __, context: any) => {
      if (context?.previousDiscussion) {
        queryClient.setQueryData(["discussion", context.previousDiscussion.id], context.previousDiscussion);
      }
    },
    onSuccess: (data) => {
